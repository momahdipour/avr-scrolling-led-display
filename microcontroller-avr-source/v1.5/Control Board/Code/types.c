...
typedef struct
{
  ...
  unsigned int dummy1;  //1 byte may be used as StageID  --  set to 0 for compatibility with future version
  unsigned int dummy2;  //set to 0 for compatibility with future version
  TArea Areas[4];
} TDisplayStage;
...
typedef struct
{
  ...
  */
  ...
  unsigned char ScrollStep;  //depends on the text scroll speed and is set by LDC program automatically
