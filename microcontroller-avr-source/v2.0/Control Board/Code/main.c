...
/*
void Alert_ms(unsigned char BlinkTimes, unsigned int ms)
{
  unsigned char i;
...
interrupt [TIM0_COMP] void timer0_comp_isr(void)
{
  ...
  #ifdef  _FULL_LATCH_
    //In full latch mode, NO NEED TO CLEAR LATCHES BEFORE GOING TO NEXT ROW
  #else
  ...
  #ifdef  _FULL_LATCH_
    //In full latch mode, NO NEED TO DISABLE THE OLD LATCH
  #else
  ...
  #ifdef  _FULL_LATCH_
    //In full latch mode, FIRST PUT NEW ROW DATA, THEN SEND A PULSE ON THE ACTIVE ROW, so don't select new row here
  #else
  ...
  #ifdef  _FULL_LATCH_
    //NOW SEND A PULSE ON THE NEW ROW (0-1-0)
    #ifdef  _COLOR_DISPLAY_METHOD_2_
    RowPulse(ActualLatchRowNum);
    #else
    RowPulse(RefreshRowNum);
    #endif
  #endif
  ...
  #ifdef  _FULL_LATCH_
    //In full latch mode, NO NEED TO CALCULATE THE VALUE OF LatchToDisable because there is NO NEED TO DISABLE THE OLD LATCH
  #else
...
/*
void delay_ms_2(unsigned int ms)
{
  unsigned long OldTick = TickCount;
  while((MSElapsed(OldTick) < ms))
  {
    ;
  }
}
...
/*
  ...
  */
  ...
  /*while(1)
  {
    oldtick = TickCount;
    delay_ms(100);
    printf("<100:%d>", TickCount - oldtick);
    
    oldtick = TickCount;
    delay_ms(10);
    printf("<10:%d>", TickCount - oldtick);

    oldtick = TickCount;
    delay_ms(500);
    printf("<500:%d>", TickCount - oldtick);
  }
  while(1)
  {
    for(c = 1; c < 10; c++)
