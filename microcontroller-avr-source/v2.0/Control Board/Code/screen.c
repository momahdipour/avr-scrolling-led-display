...
//Off time
//if OffTimeHourFrom
eeprom BOOL OffTimeDefined = FALSE;
eeprom unsigned char OffTimeHourFrom = 0;
eeprom unsigned char OffTimeHourTo = 0;
eeprom unsigned char OffTimeMinuteFrom = 0;
eeprom unsigned char OffTimeMinuteTo = 0;

eeprom unsigned char TextScrollStepSlow = 1;
eeprom unsigned char TextScrollStepMedium = 1;
eeprom unsigned char TextScrollStepFast = 2;
eeprom unsigned char TextScrollStepVeryFast = 3;
...
unsigned int MSElapsed(unsigned long oldtick);
...
unsigned char CalculateSLSTPlayDelay(unsigned char PlaySpeed)
{
  //returns in milliseconds
  if(PlaySpeed <= 64)  //Very Fast
    return 4;
  else if(PlaySpeed <= 96)  //Fast
    return 10;
  else if(PlaySpeed <= 128)  //Medium
    return 20;
  else  //Slow
    return 30;
}

#define   MAX_SLST_ANIM_DELAY    70.0  //in milliseconds
#define   MIN_SLST_ANIM_DELAY    10  //in milliseconds
unsigned char CalculateSLSTAnimPlayDelay(unsigned char AnimSpeed)
{
  //returns in milliseconds
  float res = ((float)AnimSpeed * MAX_SLST_ANIM_DELAY) / 255.0;
  unsigned char res_char = (unsigned char) res;
  if(res_char == 0)
    res_char = MIN_SLST_ANIM_DELAY;
  return res_char;
}

unsigned char CalculateFramedAnimationPlayDelay(unsigned char PlaySpeed)
{
  //returns in milliseconds
  if(PlaySpeed <= 32)  //Very Fast
    return 10;
  else if(PlaySpeed <= 64)  //Fast
    return 30;
  else if(PlaySpeed <= 128)  //Medium
    return 60;
  else  //Slow
...
#define   MAX_PAGE_EFFECT_ANIM_DELAY    50.0  //in milliseconds
#define   MIN_PAGE_EFFECT_ANIM_DELAY    10.0  //in milliseconds
unsigned char CalculatePageEffectAnimationPlayDelay(unsigned char AnimSpeed)
{
  //returns in milliseconds
  float res = ((float)AnimSpeed * MAX_PAGE_EFFECT_ANIM_DELAY) / 255.0;
  if((unsigned char) res == 0)
    res = MIN_PAGE_EFFECT_ANIM_DELAY;
  return (unsigned char) res;
}

float MSElapsed_unit_ms;

unsigned int MSElapsed(unsigned long oldtick)
{
  //MSElapsed_unit_ms = 1.024 * (float) (OVERALL_SPEED_SETTING - 4) / 16.0;  //number of milliseconds per tick  --> This value is always updated automatically whenever OVERALL_SPEED_SETTING is changed (to reduce calculation time)
  if(TickCount == oldtick)
    return 0;
  return (unsigned int) (MSElapsed_unit_ms * (TickCount - oldtick));
}
...
void ChangeStage(unsigned char NewStageIndex, BOOL FirstRun)
{
  ...
  */
  ...
  StageSettings.AreaCount = 4;
  CompatibleWithTextTimingStyle = TRUE;
  StageSettings.TimingStyle = NormalTimingStyle;
  for(i = 0; i < 4; i++)
  {
    ...
    StageSettings.ScreenFilledAtStart[i] = FALSE;
    
    if(CurrentStage.Areas[i].ContentType != 3)  //ScrollingText
      StageSettings.CompatibleWithTextTimingStyle = FALSE;
    
    switch(CurrentStage.Areas[i].ContentType)
    {
      #ifdef  _TIME_ACTIVE_
      case 1:  //Time
        CompatibleWithTextTimingStyle = FALSE;
      ...
      case 4:  //Framed Animation
        ...
        StageSettings.FA[i].Delay = -1;  //means that the frame is not loaded yet
        break;
      #endif
      case 5:  //Font Text
        break;
  ...
  StageSettings.CompatibleWithTextTimingStyle = CompatibleWithTextTimingStyle;
  if(StageSettings.CompatibleWithTextTimingStyle)
  {
    //check  to see if all text areas have the same speed
    slst_speed = -1;
    for(i = 0; i < 4; i++)
      if(CurrentStage.Areas[i].ContentType == 3)
      {
        get_content_settings(i, sizeof(TSingleLineScrollingText), (unsigned char *) &slst);
        if(slst_speed == -1)
        {
          slst_speed = slst.Speed;
        }
        else
        {
          if(slst.Speed != slst_speed)
          {
            StageSettings.CompatibleWithTextTimingStyle = FALSE;
            break;
          }
...
unsigned char GetActualTextScrollStep(unsigned char slstScrollStep)
{
  unsigned char step = 1;
  if(slstScrollStep == 1)  //Slow
    step = TextScrollStepSlow;
  else if(slstScrollStep == 2)  //Medium
    step = TextScrollStepMedium;
  else if(slstScrollStep == 3)  //Fast
    step = TextScrollStepFast;
  else if(slstScrollStep == 4)  //Very Fast
    step = TextScrollStepVeryFast;
  return step;
}
...
/*
unsigned long slst_Data_AbsoluteAddress(unsigned char dp_index, unsigned long int offset)
{
  unsigned long res = data_pointers_addr[dp_index] + (unsigned long) sizeof(TSingleLineScrollingText) + offset;
  return res;
}
*/

//#define   TEXT_SCROLL_STEP    1  //Always 1
...
//  unsigned int TotalRefreshTimems;  //in milliseconds
  TSingleLineScrollingText slst;
  unsigned int i, NextCol, StartCol, OriginalStartCol;
  ...
  //unsigned char ScrollStep;  --> replaced with TEXT_SCROLL_STEP
  unsigned char TEXT_SCROLL_STEP;
  ...
  TEXT_SCROLL_STEP = GetActualTextScrollStep(slst.ScrollStep);
  /*OLD
  if(TEXT_SCROLL_STEP == 1 || TEXT_SCROLL_STEP == 2)  //Slow and Medium speeds
  {
    TEXT_SCROLL_STEP = 1;
  }
  else  //Fast and Very Fast speeds
  {
    BytesPerCol = TEXT_SCROLL_STEP + SCROLL_STEP_ADJUST;  //**BytesPerCol is used as a temp variable here
    if(BytesPerCol > TEXT_SCROLL_STEP)  //Prevent overflow in 8-bit addition
      TEXT_SCROLL_STEP = BytesPerCol;
    if(TEXT_SCROLL_STEP == 0)
      TEXT_SCROLL_STEP = 2;
  }
  */
  //usart_putnumuc(TEXT_SCROLL_STEP);
  //usart_putnumuc(slst.ScrollStep);
//  slst.FixedText = 1;
  ...
  if(!StageSettings.EntranceAnimDone[AreaIndex])
  {
    ...
    else
    {
      if(StageSettings.AnimInitialized[AreaIndex])
      {
        ...
        BytesPerCol = CalculateSLSTAnimPlayDelay(Area.AnimSpeed);  //BytesPerCol is used here as a temp variable
        if(MSElapsed(StageSettings.LastTickCount[AreaIndex]) < BytesPerCol)  //&& StageSettings.EntranceAnimDone[AreaIndex])  //No Entrance Animation is applied to a stage
        {
          return;
        }
        ////////
        ...
        if(StageSettings.EntranceAnimDone[AreaIndex])
          StageSettings.ScreenFilledAtStart[AreaIndex] = FALSE;
        if(StageSettings.EntranceAnimDone[AreaIndex])
...
//  delay_ms(400);
  ...
  if(!StageSettings.ForceUpdate && StageSettings.EntranceAnimDone[AreaIndex])
  {
    BytesPerCol = CalculateSLSTPlayDelay(slst.Speed);  //**BytesPerCol is used as a temp variable here
    if(MSElapsed(StageSettings.LastTickCount[AreaIndex]) < BytesPerCol)
      return;
  }
  
  /*
  if(StageSettings.AreaCount == 1)
  {
    delay_ms(CalculateSLSTPlayDelay(slst.Speed));
  }
  else
  {
    if(!StageSettings.ForceUpdate && StageSettings.EntranceAnimDone[AreaIndex])
    {
      unsigned long DelayTickCount = (unsigned long)((float)slst.Speed * CONTENT_SPEED_MULTIPLIER);
//       if((TickCount - StageSettings.LastTickCount[AreaIndex]) < DelayTickCount)
//         return;
      if( (DelayTickCount <= 25) || ((TickCount - StageSettings.LastTickCount[AreaIndex]) > (DelayTickCount - 25)) )
      {
        while((TickCount - StageSettings.LastTickCount[AreaIndex]) < DelayTickCount)
        {
          ;
        }
      }
  ...
  if(StartCol == 0)
    ...
    StartCol += TEXT_SCROLL_STEP;//ScrollStep;
  ...
  if(slst.Direction == 0)  //Right
  {
    ...
    if((slst.ScrollType == 0) && !StageSettings.SLST[AreaIndex].NormalScroll && (StartCol < NextCol))
    {
      ...
      if( (StartCol + TEXT_SCROLL_STEP/*ScrollStep*/) >= NextCol)
      {
  ...
  else if(slst.Direction == 1)  //Left
  {
    if(slst.FixedText)
    {
      ...
      if((slst.ScrollType == 0) && !StageSettings.SLST[AreaIndex].NormalScroll && (StartCol < BytesPerCol /* **BytesPerCol is used as a temp variable here*/))
      {
        ...
        if( (StartCol + TEXT_SCROLL_STEP/*ScrollStep*/) >= BytesPerCol)//(AreaWidth - (AreaWidth - slst.ColCount) / 2))
        {
          StageSettings.SLST[AreaIndex].NormalScroll = TRUE;
          if(slst.FixedText == 1)
            reset_AreaElapsedTime(AreaIndex);
    ...
    else
    {
      ...
      if((slst.ScrollType == 0) && !StageSettings.SLST[AreaIndex].NormalScroll && (StartCol < AreaWidth))
      {
        ...
        if( (StartCol + TEXT_SCROLL_STEP/*ScrollStep*/) >= AreaWidth )
        {
          StageSettings.SLST[AreaIndex].NormalScroll = TRUE;
          if(slst.FixedText == 1)
            reset_AreaElapsedTime(AreaIndex);
  ...
  //MemUsedStartingAddress = slst_Data_AbsoluteAddress(dp_index, d_offset + (long) BytesPerCol);
  ...
  for(i = StartCol - 1; i < slst.ColCount; i++)
  {
    ...
    if( (TEXT_SCROLL_STEP == 1) &&
        (
        (slst.Direction == 0 && ScreenCol > Area.x1)    ||
        (slst.Direction == 1 && ScreenCol < Area.x2)
        )
        && StageSettings.ScreenFilledAtStart[AreaIndex])
    {
      //get data from the screen
      if(slst.FixedText && OriginalStartCol != StartCol)
      {
        c = LoadFromScreen(ScreenCol);
      }
      else
      {
        if(slst.Direction == 0)  //Right
        {
          c = LoadFromScreen(ScreenCol - 1);
        }
        else //if(slst.Direction == 1)  //Left
        {
          c = LoadFromScreen(ScreenCol + 1);
  ...
  StageSettings.ScreenFilledAtStart[AreaIndex] = TRUE;
...
void LEDFramedAnimation(unsigned char AreaIndex, TArea Area, unsigned char dp_index /*data pointer index*/)
{
  ...
  if(!StageSettings.ForceUpdate)// && StageSettings.EntranceAnimDone[AreaIndex])
  {
    if(fa.UseGIFTimings != 0)  //Use GIF Timings
    {
      if(StageSettings.FA[AreaIndex].Delay >= 0)
      {
        //The frame is loaded and is waiting to be displayed
        if(MSElapsed(StageSettings.LastTickCount[AreaIndex]) < StageSettings.FA[AreaIndex].Delay)
          return;
      }
    }
    else
    {
      Col = CalculateFramedAnimationPlayDelay(fa.Speed);  //**Col is used as a temp variable here
      if(MSElapsed(StageSettings.LastTickCount[AreaIndex]) < Col)
        return;
    }
  }
  ...
  */
  ...
  #pragma warn-
  FrameDataIndex = ((unsigned long) StageSettings.FA[AreaIndex].CurrentFrame - 1) * ( (unsigned long) FRAME_HEADER_SIZE + (unsigned long) fa.FrameColCount * (unsigned long) fa.BytesPerCol);
  #pragma warn+
  
  if(fa.UseGIFTimings != 0)  //Use GIF Timings
  {
    //Read new frame header
    c = GetData(fa_Data, dp_index, FrameDataIndex + 1, 1);  //read MSB byte
    c = (c << 8) | GetData(fa_Data, dp_index, FrameDataIndex, 1);  //read LSB byte
    StageSettings.FA[AreaIndex].Delay = (int) c;
  }
     ...
     fa.FrameCount > 1)
  {
    ...
    {
      b1 = FrameDataIndex + (unsigned long) FRAME_HEADER_SIZE + (unsigned long) Col * fa.BytesPerCol;
...
BOOL IsTimeInRange(unsigned char Hour, unsigned char Minute,
  unsigned char HourFrom, unsigned char HourTo, unsigned char MinuteFrom, unsigned char MinuteTo)
{
  if(HourFrom <= HourTo)
  {
  ...
  if(HourFrom == Hour)
  {
    if(MinuteFrom > Minute)
      return(FALSE);
  }
  if(HourTo == Hour)
  {
    if(MinuteTo <= Minute)  //For example, when we want the show to be played from 08:00 to 08:30, in fact when time reaches 08:00 the show must be strated and as soon as it reaches 08:30, it must be stopped. So when Hour is equal to HourTo, we stop playing if Minute is less than or equal to MinuteTo.
      return(FALSE);
  }
...
#ifdef  _TIME_SPAN_ACTIVE_
BOOL ValidateStageTimeSpan(unsigned char Hour, unsigned char Minute)
{
  unsigned int Year;
  unsigned char Month, Day;
  //Check date setting if available
  if(CurrentStage.Year > 0)
  {
    //Solar date
    get_date(&Year, &Month, &Day, 1);
    if(Year != CurrentStage.Year || Month != CurrentStage.Month || Day != CurrentStage.Day)
      return FALSE;  //not in range
  }
  //Check time span
  return IsTimeInRange(Hour, Minute, CurrentStage.HourFrom, CurrentStage.HourTo, CurrentStage.MinuteFrom, CurrentStage.MinuteTo);
}
#endif

#ifdef  _TIME_SPAN_ACTIVE_
BOOL LEDDisplayOFF(unsigned char Hour, unsigned char Minute)
{
  if(!OffTimeDefined)
    return FALSE;
  return IsTimeInRange(Hour, Minute, OffTimeHourFrom, OffTimeHourTo, OffTimeMinuteFrom, OffTimeMinuteTo);
}
#endif
...
BOOL SelectStage()
{
  ...
  if(StageSettings.TimeSpanDefined)
  {
    ...
    {
      if(!ValidateStageTimeSpan(Hour, Minute))
      {
        for(i = 0; i < 4; i++)
          StageSettings.AreasDone[i] = TRUE;
...
void DoUpdateScreenOnlyText()
{
  ...
  TArea Area;
  unsigned char dp_index /*data pointer index*/;
  unsigned char AreaIndex;
  unsigned char PlayDelay = 0;
  //BOOL UpdateAnim = FALSE;
  ...
  for(AreaIndex = 0; AreaIndex < 4; AreaIndex++)  //Main Text update loop
  {
    if(CurrentStage.Areas[AreaIndex].ContentType != 3)  //not a text content
      continue;
    
    ResetTextPlay = FALSE;
    
    dp_index = AreaIndex;
    Area = CurrentStage.Areas[AreaIndex];
    ...
    //slst = (TSingleLineScrollingText flash*) DataStart;
    get_content_settings(dp_index, sizeof(TSingleLineScrollingText), (unsigned char *) &slst);
    PlayDelay = CalculateSLSTPlayDelay(slst.Speed);
    ...
    */
    ...
    //It is assumed that the animation of the all areas is done before entering this routine
    ...
    for(i = StartCol - 1; i < slst.ColCount; i++)
    {
      ...
      b1 = LoadFromScreen(ScreenCol);  //ok
...
/*  do
  ...
  */
  ...
  if(!StageSettings.StageEffectDone)
  {
    //Timing: Page Effect
    ///*
    if(MSElapsed(StageSettings.EffectLastTickCount) < CalculatePageEffectAnimationPlayDelay(CurrentStage.EffectSpeed))
    {
      return;
    }
    ///*
  ...
  //Check to select the best timing style
  if(StageSettings.CompatibleWithTextTimingStyle)
  {
    //Check if all stages' animations is done and normal scroll is started
    AnimsDone = TRUE;
    for(i = 0; i < 4; i++)
    {
      if(CurrentStage.Areas[i].ContentType == 3)
      {
        AnimsDone = AnimsDone && StageSettings.EntranceAnimDone[i];
      }
    }
    if(AnimsDone)
      StageSettings.TimingStyle = OnlyTextTimingStyle;
  }
  
  
  if(StageSettings.TimingStyle == OnlyTextTimingStyle)
  {
    DoUpdateScreenOnlyText();
    StageSettings.TimingStyle = NormalTimingStyle;
  }
  else if(StageSettings.TimingStyle == NormalTimingStyle)
  {
