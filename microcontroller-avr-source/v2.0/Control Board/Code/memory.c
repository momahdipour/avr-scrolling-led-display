...
//unsigned long MemUsedStartingAddress = 0;
...
eeprom unsigned int EEPROM_IC_Size_EEP = 65536-1;//2048 = AT24C16, 4096 = AT24C32, 8192 = AT24C64, 65536-1 = AT24C512  //AT24C64 - in bytes - can be set via usart command usart_cmd_SetEEPROMs - AT24C32 = 4KB (minimum reasonable memory)
unsigned int EEPROM_IC_Size;
unsigned char EEPROM_IC_PAGE_SIZE;
...
unsigned long GetMemSize()
{
  ...
  return((unsigned long) EEPROM_IC_Count_EEP * (unsigned long) EEPROM_IC_Size_EEP);
}
...
void mem_init()
{
  ...
  if(EEPROM_IC_Size <= 2048)
    ...
    EEPROM_IC_PAGE_SIZE = MAX_EEPROM_IC_PAGE_SIZE;  //=128; Greater than the value of MAX_EEPROM_IC_PAGE_SIZE is not allowd
...
unsigned char mem_find_unused_cache()
{
  ...
  //BOOL cache_looped = FALSE;
  
  /*New Code using MemUsedStartingAddress (NOT TESTED AT ALL)
  while(TRUE)
  {
    replaced_cache++;
    if(replaced_cache >= (CACHE_COUNT - 1))  //last cache is used as temp cache
    {
      if(cache_looped)
    ...
    //if(MemUsedStartingAddress < caches[replaced_cache].start_addr ||
    //   MemUsedStartingAddress >= (caches[replaced_cache].start_addr + CACHE_SIZE))
    if(caches[replaced_cache].start_addr >= MemUsedStartingAddress)
    {
      continue;
    }
    if((caches[replaced_cache].start_addr + CACHE_SIZE) > MemUsedStartingAddress)
    {
      continue;
    }
